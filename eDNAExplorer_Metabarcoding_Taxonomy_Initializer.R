      ...
      tryCatch(
        {
          ...
          if (length(TronkoFiles) > 0) {
            ...
            Retained_Columns <- c(
              "ProjectID", "Primer", "Taxonomic_Path", "Score", "SampleID", "superkingdom", "kingdom",
        ...
        error = function(e) {
          # This code will run if an error occurs in the try block
          # Append the failed Primer to the FailedPrimer list
          FailedPrimer[[length(FailedPrimer) + 1]] <- list(Primer = Primer, Error = e)
          # Log the error to Sentry
          with_sentry({
            # Adding context information
            sentry_set_tag("project_id", ProjectID)

            # Capture the exception
            capture_exception(e, extra = list(Primer = Primer))

            # Optionally, include the project ID in the message as well
            sentry_set_context("failed_primer_details", list(Primer = Primer, ProjectID = ProjectID))
          })

          print(paste("Error for primer:", Primer, "; Error:", e$message))
        }
      )
    }
